...
cigs_smoked_fup3 <- data.frame(FollowUp = "Follow-Up 3", Response = fup3$cigs_smoked_3)
# Combine and remove NA values
combined_data_cigs <- rbind(cigs_smoked_base, cigs_smoked_fup1, cigs_smoked_fup2,cigs_smoked_fup3 )
...
# Add the number of each follow-up at the top of the stacked bar
p_cigs_bar <- p_cigs_bar +
  geom_text(aes(label = stat(count)), stat = "count", vjust = -0.5, size = 3, position = position_stack(vjust = 0.5))
